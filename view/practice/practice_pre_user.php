<?php
    ...
    //メールアドレス空欄の場合
        if (empty($_POST['mail'])) {
            ...
            //DB確認        
            $sql = "SELECT id FROM practice.user WHERE mail=:mail";
            $stm = $pdo->prepare($sql);
   ...
   //エラーがない場合、pre_userテーブルにインサート
    if (count($errors) === 0){
        ...
        $url = "http://localhost/ST2109005_kawamura/view/practice_signup.php?urltoken=".$urltoken;
        //ここでデータベースに登録する
        try{
            //例外処理を投げる（スロー）ようにする
